"""
VAE with prev_action conditioning, trained with dropout on prev_action.
30% of samples get prev_action=zeros → VAE learns to handle missing feedback.
"""
import sys, os
...
LR = 1e-4
SAVE_EVERY = 200
OUTPUT_DIR = 'outputs/vae_feedback_v2'
PREV_DROPOUT = 0.3  # 30% samples have prev_action zeroed
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    for batch in train_loader:
        ...
        # Dropout: randomly zero out prev_action
        if torch.rand(1).item() < PREV_DROPOUT:
            prev_action = torch.zeros_like(prev_action)

        loss, info = model.get_loss(
            {'obs': obs, 'action': action, 'prev_action': prev_action},
    ...
    if epoch % 50 == 0:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                action = nbatch['action'].to(DEVICE).float()
                prev_action = nbatch['prev_action'].to(DEVICE).float()
                # Validate both with and without prev_action
...
torch.save({'model': model.state_dict(), 'epoch': NUM_EPOCHS}, os.path.join(OUTPUT_DIR, 'final.pt'))
