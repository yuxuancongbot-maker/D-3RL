...
ckpt = ROOT + '/outputs/square/checkpoints/epoch=0149-test_mean_score=0.900.ckpt'
...
rcfg = OmegaConf.create({'_target_':'diffusion_policy.env_runner.robomimic_lowdim_runner.RobomimicLowdimRunner',
    'output_dir':'/tmp/ev_sq149', 'dataset_path': ROOT + '/data/robomimic/datasets/square/ph/low_dim_abs.hdf5',
...
for ri, start in enumerate([100000, 100050, 100100]):
    ...
    print(f'Square ep149 run{ri+1}: succ={np.mean(ss):.1%} avg_k={np.mean(kk):.2f} lat={np.mean(ll):.1f}ms', flush=True)
    ss_all.append(np.mean(ss)); kk_all.append(np.mean(kk)); ll_all.append(np.mean(ll))
env.close()
print(f'Square ep149 FINAL: succ={np.mean(ss_all):.1%}±{np.std(ss_all):.1%} avg_k={np.mean(kk_all):.2f}±{np.std(kk_all):.2f} lat={np.mean(ll_all):.1f}±{np.std(ll_all):.1f}ms', flush=True)
